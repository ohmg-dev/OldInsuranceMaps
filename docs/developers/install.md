...
The frontend incorporates many independently built [Svelte](https://svelte.dev) components, using [OpenLayers](https://openlayers.org) for all map interfaces. [OpenStreetMap](https://openstreetmap.org) and [Mapbox](https://www.mapbox.com) are the basemap sources.
...
You can also use the `--configComponent` argument to only build the specific component(s) that you are working on, which greatly improve the development experience. You can pass that argument a comma-separated list of component names as well:

```bash
pnpm run dev --configComponent Georeference,Split
```
