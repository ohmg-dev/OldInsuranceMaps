...
from lc_insurancemaps.utils import enumerations, parsers
...
class MapCollectionItem(Document):
    ...
    def parse_location_infoOLD(self, item):
        ...
        # kind of elaborate parsing of location tags
        # first find the state and then remove that tag from the list
        state_position = None
        for index, l in enumerate(locations):
            if l.lower() in [i[0].lower() for i in enumerations.STATE_CHOICES]:
                info["state"] = full_capitalize(l)
                state_position = index
        if state_position is not None:
            del locations[state_position]

        # next find the county/parish and remove that from the list
        county_position = None
        for index, l in enumerate(locations):
            l_words = [i.lower() for i in l.split(" ")]
            if "county" in l_words or "parish" in l_words:
                info["county"] = full_capitalize(l)
                county_position = index
        if county_position is not None:
            del locations[county_position]

        # finally compare remaining tag(s) to the title and only the city name
        # should match
    ...
    def parse_date_tagOLS(self, date_tag):
