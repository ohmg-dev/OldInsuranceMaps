...
import MapboxLogoLink from "../components/interfaces/buttons/MapboxLogoLink.svelte"
	...
	watchId = navigator.geolocation.watchPosition(
		function (pos) {
			...
			gpsSource.addFeatures([
				new Feature(
					accuracy.transform('EPSG:4326', viewer.map.getView().getProjection())
				),
...
function locateUser() {
	if (!watchId) {
		enableGPSLocation();
	} else if (!gpsSource.isEmpty() && viewer.map) {
		viewer.map.getView().fit(gpsSource.getExtent(), {
		maxZoom: 18,
		duration: 500,
		});
	}
...
function resetExtent() {
	viewer.map.getView().fit(homeExtent);
...
onMount(() => {
  ...
  viewer = new MapViewer("map");
	...
	if (homeExtent) {
		...
		if (MAPS.length > 0) {
			viewer.map.getView().setMinZoom(14)
		}
		viewer.map.getView().fit(homeExtent)
		// set initial zoom to integer to improve tile efficiency, esp. when
		// user zooms in and out with mouse wheel or clicks/taps
		viewer.map.getView().setZoom(Math.round(viewer.map.getView().getZoom()))
		viewer.map.getView().setMinZoom(0)
	}
	volumeIds.forEach(function (vol) {
		if (volumeLookup[vol].mainLayer) {
			viewer.map.addLayer(volumeLookup[vol].mainLayer)
		}
	})

	viewer.map.addLayer(gpsLayer);

	viewer.map.addControl(
		new Control({
	...
	if (CONTEXT.on_mobile){
		viewer.map.on('singleclick', function() { showPanel = !showPanel })
	}
	sync(viewer.map);
