<script>
  ...
  $: {
    loading = true;
    let fetchUrl = `/api/beta2/contributors/?offset=${offset}`;
    if (limit != 0 && useLimit) {
  ...
  function toggleList(userId) {
    ...
    listBtnEl.innerText = listBtnEl.innerText == 'show list' ? 'hide list' : 'show list';
  }
</script>
  ...
  <div style="height: 100%; overflow-y:auto; border:1px solid #ddd; border-radius:4px; background:white;">
    {#if items.length > 0}
      <table>
        <thead>
          <tr>
            <th><SortButton title="Username" bind:sortDir bind:sortParam bind:offset value={'username'} /></th>
            <th><SortButton title="Loaded maps" bind:sortDir bind:sortParam value={'year'} /></th>
            <th
              ><SortButton
                title="Docs"
        ...
        <tbody>
          {#each items as s}
            <tr style="height:38px; vertical-align:center;">
              <td><Link href={`/profile/${s.username}`}>{s.username}</Link></td>
              <td
                ><div>
                  {s.load_ct}{#if s.maps.length > 0}&nbsp;&mdash;{/if}
                  <!-- {#each p.maps as v, n}
                {#if n <= 2}
              ...
              {/each} -->
                  {#if s.maps.length > 0}
                    <button id="{s.username}-btn" title="Show loaded maps" on:click={() => toggleList(s.username)}
                      >show list</button
                    >
                  {/if}
                </div>
                {#if s.maps.length > 0}
                  <div id={s.username} class="full-volume-list">
                    <ul>
                      {#each s.maps as v}
