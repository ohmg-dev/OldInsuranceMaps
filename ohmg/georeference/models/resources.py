...
class ItemBase(models.Model):
    ...
    x0 = models.DecimalField(max_digits=30, decimal_places=15, blank=True, null=True)
    y0 = models.DecimalField(max_digits=30, decimal_places=15, blank=True, null=True)
    x1 = models.DecimalField(max_digits=30, decimal_places=15, blank=True, null=True)
    y1 = models.DecimalField(max_digits=30, decimal_places=15, blank=True, null=True)
    ...
    def extent(self):
        ...
        if self.x0 is not None:
            extent = (float(self.x0), float(self.y0), float(self.x1), float(self.y1))
        return extent
    ...
    def update_annotationset(self, vrs):
        ...
        if self.vrs:
            if self.vrs.multimask and self.slug in self.vrs.multimask:
                ...
                logger.warning(
                    f"{self.pk} removed layer from existing multimask in vrs {self.vrs.pk}"
                )
        self.vrs = vrs
...
class Document(ItemBase):
    ...
    def serialize(
        ...
        serialize_parent=True,
        serialize_layer=True,
        ...
        if self.parent is not None:
            if serialize_parent:
                parent = self.parent.serialize(
                    serialize_children=False, serialize_layer=serialize_layer
                )
            else:
        ...
        if len(self.children) > 0:
            if serialize_children:
                children = [
                    i.serialize(serialize_parent=False, serialize_layer=serialize_layer)
